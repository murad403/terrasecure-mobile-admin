...
import { X, Loader2, MapPin, User, CheckCircle, FileText, Compass, Calendar, Layers, ShieldCheck, UploadCloud, UserCheck, Edit3, Trash2 } from 'lucide-react'
import { useRetrieveSurveyDetailsQuery } from '@/redux/features/survey/survey.api'
...
const SurveyDetailsModal: React.FC<SurveyDetailsModalProps> = ({
  ...
  const { data: detailsRes, isLoading } = useRetrieveSurveyDetailsQuery(
    surveyId!,
      ...
      <div className="relative w-full sm:w-140 md:w-160 h-full bg-white shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-right duration-300 ease-out border-l border-slate-100 z-50">
        ...
        {isLoading ? (
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-none">
            ...
            <div className="flex items-center justify-between p-3 bg-slate-50 border border-slate-200/80 rounded-xl flex-wrap gap-2">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {onAssignSurveyor && (
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    onClick={() => onAssignSurveyor(survey)}
                    className="text-xs gap-1.5 border-slate-200"
                  >
                ...
                {onVerifyGis && (
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    onClick={() => onVerifyGis(survey)}
                    className="text-xs gap-1.5 border-slate-200"
                  >
                    <ShieldCheck className="w-3.5 h-3.5 text-emerald-600" />
                    <span>Verify GIS</span>
                  </Button>
                )}
                {onEditSurvey && (
                  <Button
