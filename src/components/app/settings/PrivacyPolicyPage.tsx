...
import { useGetPrivacyPolicyQuery, useUpdatePrivacyPolicyMutation } from '@/redux/features/auth/auth.api'
import RichTextEditor from '@/components/shared/RichTextEditor'
import { toast } from 'sonner'

const PrivacyPolicyPage = () => {
  const { data: getRes, isLoading, isError, refetch } = useGetPrivacyPolicyQuery()
  ...
  const handleSave = async () => {
    try {
      const res = await updatePrivacyPolicy({ data: content }).unwrap()
      if (res.success) {
      ...
      toast.error(err?.data?.message || 'Error updating Privacy Policy.')
    }
  }
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm space-y-6 relative">
      ...
      <div className="flex items-center justify-between border-b border-gray-100 pb-4">
        <div>
          <h2 className="text-lg font-bold text-gray-900 leading-tight">Privacy Policy</h2>
          <p className="text-xs text-gray-500 font-light">
            Manage data protection, privacy terms, and governance policies.
          </p>
        </div>
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-16 space-y-3">
          ...
          <p className="text-xs font-semibold text-slate-400">Loading Privacy Policy...</p>
        </div>
      ) : isError ? (
        <div className="flex flex-col items-center justify-center py-12 text-center space-y-3">
          <p className="text-sm font-semibold text-rose-500">Failed to load Privacy Policy.</p>
          <button
            type="button"
            onClick={() => refetch()}
        ...
        <div className="border border-gray-100 rounded-xl p-6 min-h-80 bg-slate-50/30">
          {getRes?.data ? (
            ...
            <div className="flex items-center justify-center h-48 text-xs font-semibold text-slate-400">
              No Privacy Policy content found. Click "Edit Policy" to add content.
            </div>
          )}
        </div>
      )}
